...
def visualize_voxels(v, fname = "q1-0-src.gif", image_size = 512, dist = 3, elev = 0):
    ...
    cameras = pytorch3d.renderer.FoVPerspectiveCameras(R=R, T=T, device=device)
    rend = renderer(myMesh, cameras=cameras, lights=lights)
    rend = rend[0, ..., :3].detach().cpu().numpy().clip(0, 1)
    plt.imsave("results/q1_1_res.jpg", rend)

    render_360d(mesh = myMesh, fname = fname, device=device, image_size=image_size, dist=dist, elev=elev, clip=True)
...
# Q1.3
def visualize_mesh(myMesh, fname = "q1_3.gif", image_size = 512, dist = 1, elev = 0):
    device = get_device()
    vertices = myMesh.verts_list()[0]
    faces = myMesh.faces_list()[0]
    textures = (vertices - vertices.min()) / (vertices.max() - vertices.min())
